...
- Go outside the Gemma family to the true frontier, and the numbers jump again: as a rough rule of thumb, a 70-billion-parameter model needs around 40GB of memory to run well — more than any consumer graphics card has on its own (high-end consumer GPUs typically top out around 24GB).
